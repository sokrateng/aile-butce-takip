...
function App() {
    ...
    const actualIncome = allMonthTransactions.filter(t => t.type === 'income' && !t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
    const estimatedIncome = allMonthTransactions.filter(t => t.type === 'income' && t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
    const actualExpense = allMonthTransactions.filter(t => t.type === 'expense' && !t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
    const estimatedExpense = allMonthTransactions.filter(t => t.type === 'expense' && t.is_estimated).reduce((acc, c) => acc + c.amount, 0);

    const totalIncome = actualIncome + estimatedIncome;
    const totalExpense = actualExpense + estimatedExpense;
    const actualBalance = actualIncome - actualExpense;
    const balance = totalIncome - totalExpense;

    const userStats = users.map(user => {
        const userTrans = allMonthTransactions.filter(t => t.user_id === user.id);
        const income = userTrans.filter(t => t.type === 'income' && !t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
        const expense = userTrans.filter(t => t.type === 'expense' && !t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
        const estIncome = userTrans.filter(t => t.type === 'income' && t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
        const estExpense = userTrans.filter(t => t.type === 'expense' && t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
    ...
    for (let i = 3; i >= 0; i--) {
        ...
        const incomeActual = monthTrans.filter(t => t.type === 'income' && !t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
        const incomeEstimated = monthTrans.filter(t => t.type === 'income' && t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
        const expenseActual = monthTrans.filter(t => t.type === 'expense' && !t.is_estimated).reduce((acc, c) => acc + c.amount, 0);
        const expenseEstimated = monthTrans.filter(t => t.type === 'expense' && t.is_estimated).reduce((acc, c) => acc + c.amount, 0);

        monthlyComparisonData.push({
            name: format(targetDate, 'MMMM', { locale: tr }),
            gelir: incomeActual,
            gelirTahmini: incomeEstimated,
            gider: expenseActual,
            giderTahmini: expenseEstimated
        });
    }

    // Chart Data: Category Pie Charts (sadece kesinleşmiş kalemler)
    const getCategoryData = (type) => {
        const data = allMonthTransactions
            .filter(t => t.type === type && !t.is_estimated)
            .reduce((acc, t) => {
